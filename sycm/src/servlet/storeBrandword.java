...
public class storeBrandword extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			String json = brandWord.api(date, catId, parentId);
			...
			if (obj != null) {
				...
				for (JsonElement jsonElement : hotList) {
					...
					int num = 0;
					if(type.equals("1")) {
						num = service.fillBrandwordHot(catId, avg_word_click_hits, avg_word_click_rate,
							avg_word_pay_rate, avg_word_se_ipv_uv_hits, hot_search_rank, search_word, p4p_ref_price,
							rel_se_word_cnt, date, crawl_time);
					}else {
						num = service_make.fillBrandwordHot(catId, avg_word_click_hits, avg_word_click_rate,
								avg_word_pay_rate, avg_word_se_ipv_uv_hits, hot_search_rank, search_word, p4p_ref_price,
								rel_se_word_cnt, date, crawl_time);
					}
					if (num != 1) {
				...
				for (JsonElement jsonElement : soarList) {
					...
					int num = 0;
					if(type.equals("1")) {
						num = service.fillBrandwordSoar(catId, avg_word_click_hits, avg_word_se_rise_rate,
							avg_word_pay_rate, avg_word_se_ipv_uv_hits, soar_rank, search_word, p4p_ref_price,
							rel_se_word_cnt, date, crawl_time);
					}else {
						num = service_make.fillBrandwordSoar(catId, avg_word_click_hits, avg_word_se_rise_rate,
								avg_word_pay_rate, avg_word_se_ipv_uv_hits, soar_rank, search_word, p4p_ref_price,
								rel_se_word_cnt, date, crawl_time);
					}
					if (num != 1) {

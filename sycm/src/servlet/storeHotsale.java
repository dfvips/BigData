...
public class storeHotsale extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			String json = hotsale.api(date, catId, parentId);
			...
			if(obj!=null) {
				...
				for (JsonElement jsonElement : dataArray) {
					...
					String trade_growth_range = null;
					String trade_growth_range_crc = null;
					String item_id = null;
					String pict_url = null;
					String detail_url = null;
					String item_title = null;
					String b2c_shop = null;
					...
					String pay_rate_index = null;
					if(data.get("tradeGrowthRange")!=null) {
						if(data.get("tradeGrowthRange").getAsJsonObject().get("value")!=null) {
							trade_growth_range = data.get("tradeGrowthRange").getAsJsonObject().get("value").getAsString();
						}
						if(data.get("tradeGrowthRange").getAsJsonObject().get("cycleCqc")!=null) {
							trade_growth_range_crc = data.get("tradeGrowthRange").getAsJsonObject().get("cycleCqc").getAsString();
						}
					}
					...
					if(shop.get("b2CShop")!=null) {
						b2c_shop = shop.get("b2CShop").getAsString();
					}
					...
					if(data.get("payRateIndex")!=null) {
						if(data.get("payRateIndex").getAsJsonObject().get("value")!=null) {
							pay_rate_index = data.get("payRateIndex").getAsJsonObject().get("value").getAsString();
						}
					}
					int num = 0;
					if(type.equals("1")) {
						num = service.fillHotsale(catId, trade_growth_range, trade_growth_range_crc, item_id, pict_url,
							detail_url, item_title, b2c_shop, picture_url, shop_title, shop_url, user_id, cate_rank,
							cate_rank_cqc, trade_index, pay_rate_index, date, crawl_time);
					}else {
						num = service_make.fillHotsale(catId, trade_growth_range, trade_growth_range_crc, item_id, pict_url,
								detail_url, item_title, b2c_shop, picture_url, shop_title, shop_url, user_id, cate_rank,
								cate_rank_cqc, trade_index, pay_rate_index, date, crawl_time);
					}
					if (num != 1) {

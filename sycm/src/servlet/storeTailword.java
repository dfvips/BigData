...
public class storeTailword extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			String json = tailWord.api(date, catId, parentId);
			...
			if (obj != null) {
				...
				for (JsonElement jsonElement : hotList) {
					...
					int num = 0;
					if(type.equals("1")) {
						num = service.fillTailwordHot(catId, click_hits, click_rate, hot_search_rank, order_num, p4p_ref_price,
							pay_rate, se_ipv_uv_hits, search_word, tm_click_rate, date, crawl_time);
					}else {
						num = service_make.fillTailwordHot(catId, click_hits, click_rate, hot_search_rank, order_num, p4p_ref_price,
								pay_rate, se_ipv_uv_hits, search_word, tm_click_rate, date, crawl_time);
					}
					if (num != 1) {
				...
				for (JsonElement jsonElement : soarList) {
					...
					int num = 0;
					if(type.equals("1")) {
						num = service.fillTailwordSoar(catId, click_hits, click_rate, soar_rank, order_num,
								p4p_ref_price, pay_rate, se_ipv_uv_hits, search_word, se_rise_rate, date, crawl_time);
					}else {
						num = service_make.fillTailwordSoar(catId, click_hits, click_rate, soar_rank, order_num,
								p4p_ref_price, pay_rate, se_ipv_uv_hits, search_word, se_rise_rate, date, crawl_time);
					}
					if (num != 1) {

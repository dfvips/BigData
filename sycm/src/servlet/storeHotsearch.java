...
public class storeHotsearch extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			String json = hotsearch.api(date, catId, parentId);
			...
			if (obj != null) {
				...
				for (JsonElement jsonElement : dataArray) {
					...
					if (data.get("seIpvUvHits") != null) {
						if (data.get("seIpvUvHits").getAsJsonObject().get("value") != null) {
							se_ipv_uv_hits = data.get("seIpvUvHits").getAsJsonObject().get("value").getAsString();
						}
					}
					if (data.get("uvIndex") != null) {
						if (data.get("uvIndex").getAsJsonObject().get("value") != null) {
							uv_index = data.get("uvIndex").getAsJsonObject().get("value").getAsString();
						}
					}
					int num = 0;
					if(type.equals("1")) {
						num = service.fillHotsearch(catId, item_id, pict_url, detail_url, item_title, b2c_shop,
							picture_url, shop_title, shop_url, user_id, trade_index, cate_rank, se_ipv_uv_hits, uv_index,
							date, crawl_time);
					}else {
						num = service_make.fillHotsearch(catId, item_id, pict_url, detail_url, item_title, b2c_shop,
								picture_url, shop_title, shop_url, user_id, trade_index, cate_rank, se_ipv_uv_hits, uv_index,
								date, crawl_time);
					}
					if (num != 1) {

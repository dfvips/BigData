	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = 0; j < trendsArray.size(); j++) {
				...
				int num = sqlSession.getMapper(Mapper.class).queryIndustrytrend(visitorNum, searchNum, clickNum, date,
						catId);
				if (num == 0) {
					sqlSession.getMapper(Mapper.class).storeIndustrytrend(visitorNum, searchNum, clickNum, date, catId,
							catName);
					sqlSession.commit();
				}
				String res = "";
				...
				res += "{\"date\":\"" + date + "\",\"visitorNum\":" + visitorNum + ",\"searchNum\":" + searchNum
						+ ",\"clickNum\":" + clickNum + ",\"timestamp\":" + new Date().getTime() + "}";
				out.print(res);
			}
		}
		sqlSession.close();
